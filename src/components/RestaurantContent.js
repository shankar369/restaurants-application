...
const RestaurantContent = ({isAdmin, restaurant,ratings, setRatings}) => {
    ...
    const [newOne,setNewOne] = useState(restaurant?.top_rating === -1 ? true : false)
        ...
        <div className="flex-class">
            ...
            <div className="restaurant-reviews shadow">
                <div className="reviews-header">
                    <h1>Reviews</h1>
                    <button className="btn btn-primary" onClick={()=>{setShowForm(true)}}>Add Your Review</button>
                </div>
                <div className="reviews">
                    {newOne && <center><h2>No reviews yet</h2></center>}
                    {ratings && ratings.map(review => <ReviewCard setShowForm={setShowReviewDeleteForm} setReviewId={setReviewId} isAdmin={isAdmin} key={review._id} review={review} /> )}
                </div>
            </div>
            <div onClick={e => handleModalClick(e)}>
                {showForm &&<LightBox> <ReviewForm updateRatingMath={updateRatingMath} restaurant_id={restaurant._id} setRatings={setRatings} setShowForm={setShowForm} /> </LightBox>}
            </div>
            
            <div onClick={e => handleModalClick(e)}>
                {showReviewDeleteForm &&<LightBox> <ReviewDeleteForm reviewId={reviewId} setShowForm={setShowReviewDeleteForm} setReviews={setRatings} error={error} setError={setError} /> </LightBox>}
            </div>
        </div>
    )
}
